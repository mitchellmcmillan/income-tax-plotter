...
test(`${COUNTRY_LABEL} student loan increases marginal rate above threshold`, () => {
  const income = 50_000;
  const noLoan = interpreter.prepare(COUNTRY, ENABLED_SCHEDULES, CURRENCY).marginalRate(income);
  const withLoan = interpreter
    .prepare(COUNTRY, ['income tax', 'loan repayment'], CURRENCY)
    .marginalRate(income);
